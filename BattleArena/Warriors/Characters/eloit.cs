...
{
    public class eloit : Warrior, IHealCaster
    {
        public int HealingAmount { get; set; }
        public eloit(int health, int attackPower, int speed, int healingAmount, TeamType teamType)
            : base("eloit", health, attackPower, speed, WarriorType.Magery, teamType)
        {
        ...
        {
            var dmginfo = new DamageInfo(AttackPower, "ano nanaman", HasCriticalChance, this);
            TakeDamage(dmginfo);
        ...
        public void HealTeamMates(List<Warrior> teamMates)
        {
            foreach (var warrior in teamMates)
            {
                if (warrior.IsAlive && warrior.TeamType == TeamType)
                {
                    ...
                    Console.WriteLine($"->{Name}: Sayang, patay na si tanga {warrior.Name}. " +
